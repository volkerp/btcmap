    ...
    for row in rows:
        ...
        if day != last_day:
            if daystats['block_count'] > 0:
                ...
                print(f"Day: {last_day}, Transactions: {avg_transactions}, Size: {avg_size}, Minted Value: {satoshis_to_btc(avg_minted)}, Output Value: {satoshis_to_btc(avg_output)}, Difficulty: {avg_difficulty:.6f}")
                c.execute("INSERT OR REPLACE INTO days (date, num_transactions, size, minted_value, output_value, difficulty) VALUES (?, ?, ?, ?, ?, ?)",
                          (int(last_day.strftime("%Y%m%d")), avg_transactions, avg_size, avg_minted, avg_output, avg_difficulty))
